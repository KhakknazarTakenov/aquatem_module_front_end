async function checkIsUserLogined(page) {
    ...
    const userLastName = document.cookie.split('; ').find(row => row.startsWith('last_name='));
    if (!userName || !userLastName) {
        redirectToLogin();
    } else {
        const isPermited = await checkUserPeremission(page);
        if (!isPermited) {
            alert("Данный пользователь не имеет достаточных прав на просмотр данной страницы");
...
async function checkUserPeremission(page) {
    ...
    const res = await (await fetch(BASE_URL + "/check_user_permission/", {
        method: 'POST',
...
function checkWarehouseManagerPermission(permissions) {
    if (permissions === "all")
        return true;
    else if (permissions === "installation_team")
        return true;
    else
        return false;
}
...
function parseUserPermissions(permissions) {
    if (permissions.includes("warehouse_manager") && permissions.includes("installation_team")) {
        return "all"
