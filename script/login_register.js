async function login() {
    ...
    const res = await (await fetch(BASE_URL + "/login/",
        {
            method: 'POST',
    ...
    if (res.status) {
        ...
        alert(`Для пользователя ${firstName} ${lastName} еще не установлен пароль, пройдите регистрацию`)
        redirectToRegister();
    }
...
async function register() {
    ...
    const lastName = document.getElementById('last_name').value.trim();
    const password = document.getElementById('password').value.trim();
    const res = await (await fetch(BASE_URL + "/register/",
        {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
